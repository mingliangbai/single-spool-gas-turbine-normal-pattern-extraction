...
load dataab1.mat
cd ..
cd ./'NARX network with other parameter combinations'/'Narx1t1_p_156'
egtsmooth=p;%smooth(egt);%egt;%smooth(egt);
%% 建立非线性自回归模型
close all
inputSeries=num2cell([t1']);%two external inputs, fuel and t1 are column vectors of the same length
targetSeries=num2cell(egtsmooth');%NWP的预测误差
inputDelays =0;%外部输入变量1的0阶延迟；
%如何给外部输入变量1和外部输入变量2设置不同的延迟阶数还没弄明白
fdelay=3;%5-order PACF is significant larger than zero
...
%         figure; plot(fittest,'b');hold on;plot(actualtest,'r');legend('fitted','actual');title('test data')
        ...
        %% 异常检测1
        %         cd ..        
        inputSeries3=num2cell([t1ab1']);%two external inputs, fuel and t1 are column vectors of the same length
        targetSeries3=num2cell(pab1');%NWP的预测误差
        ...
        eval(['save narx_p_hidden' num2str(hiddenLayerSize) '.mat'])
end
%%
save Narx1t1_p_156.mat acctrainnorbox accvalnorbox acctestnorbox  accab1box acctrainnorsig accvalnorsig acctestnorsig  accab1sig trainmae trainmape valrmse valmae valmape testmae testmape testrmse
cd ..
