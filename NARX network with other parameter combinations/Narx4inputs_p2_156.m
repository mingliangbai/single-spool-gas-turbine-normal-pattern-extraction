...
load dataab1.mat
cd ..
cd ./'NARX network with other parameter combinations'/'Narx4inputs_p2_156'
egtsmooth=p2;%smooth(egt);%egt;%smooth(egt);
%% 建立非线性自回归模型
close all
inputSeries=num2cell([fuel';t1';p';t4']);%two external inputs, fuel and t1 are column vectors of the same length
...
for hiddenLayerSize = 1:20
    ...
    net.numInputs=5;%3个外部输入变量时，将net.numInputs设置为3+1=4即可进行正常的训练，net.numInputs比实际的Narx网络输入多1
    ...
    %% 异常检测1
    %         cd ..
    inputSeries3=num2cell([fuelab1';t1ab1';pab1';t4ab1']);%two external inputs, fuel and t1 are column vectors of the same length
