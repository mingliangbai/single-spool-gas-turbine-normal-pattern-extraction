...
feature accel on
warning off
mkdir('Narx3inputs_t1_fuel_p2_P_156');
cd ..
addpath('./toolbox')
cd ./'data'
load datanormal.mat
load dataab1.mat
cd ..
cd ./'NARX network with other parameter combinations'/'Narx3inputs_t1_fuel_p2_P_156'
egtsmooth=p;%smooth(egt);%egt;%smooth(egt);
%% 建立非线性自回归模型
close all
inputSeries=num2cell([fuel';t1';p2']);%two external inputs, fuel and t1 are column vectors of the same length
...
for hiddenLayerSize = 1:20%5:20%:20%10%12%5:20%10;
...
%         figure; plot(fittest,'b');hold on;plot(actualtest,'r');legend('fitted','actual');title('test data')
        ...
        %% 异常检测1
        %         cd ..        
        inputSeries3=num2cell([fuelab1';t1ab1';p2ab1']);%two external inputs, fuel and t1 are column vectors of the same length
