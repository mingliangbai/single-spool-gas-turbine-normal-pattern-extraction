...
feature accel on
warning off
mkdir('Narx3inputs_t1_fuel_P_t4_156');
cd ..
addpath('./toolbox')
cd ./'data'
load datanormal.mat
load dataab1.mat
cd ..
cd ./'NARX network with other parameter combinations'/'Narx3inputs_t1_fuel_P_t4_156'
egtsmooth=t4;
%% 建立非线性自回归模型
close all
inputSeries=num2cell([fuel';t1';p']);%two external inputs, fuel and t1 are column vectors of the same length
targetSeries=num2cell(egtsmooth');%NWP的预测误差
inputDelays =0;%外部输入变量1的0阶延迟；
...
for hiddenLayerSize = 1:20
        ...
        net.numInputs=4;%3个外部输入变量时，将net.numInputs设置为3+1=4即可进行正常的训练，net.numInputs比实际的Narx网络输入多1
...
%         figure; plot(fittest,'b');hold on;plot(actualtest,'r');legend('fitted','actual');title('test data')
        ...
        %% 异常检测1
        %         cd ..        
        inputSeries3=num2cell([fuelab1';t1ab1';pab1']);%two external inputs, fuel and t1 are column vectors of the same length
        targetSeries3=num2cell(t4ab1');%NWP的预测误差
...
%%
save narx_t4_156.mat acctrainnorbox accvalnorbox acctestnorbox  accab1box accab2box acctrainnorsig accvalnorsig acctestnorsig  accab1sig accab2sig trainmae trainmape valrmse valmae valmape testmae testmape testrmse
cd ..
